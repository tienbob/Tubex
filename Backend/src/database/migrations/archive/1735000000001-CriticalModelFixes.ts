...
export class CriticalModelFixes1735000000001 implements MigrationInterface {
    name = 'CriticalModelFixes1735000000001'

    public async up(queryRunner: QueryRunner): Promise<void> {
        // ============================================================================
        // CRITICAL FIX 1: Add company_id to batches table for multi-tenant isolation
        // ============================================================================

        // Add company_id column to batches table
        await queryRunner.query(`
            ALTER TABLE "batches" ADD COLUMN "company_id" UUID;
        `);
        ...
        await queryRunner.query(`
            ALTER TABLE "batches" ALTER COLUMN "company_id" SET NOT NULL;
        `);
        ...
        await queryRunner.query(`
            ALTER TABLE "batches" 
            ADD CONSTRAINT "UQ_batches_batch_number_company" 
            UNIQUE ("batch_number", "company_id");
        `);

        // ============================================================================
        // CRITICAL FIX 2: Add company_id to payments table for multi-tenant isolation
        // ============================================================================

        // Add companyId column to payments table
        await queryRunner.query(`
        ...
        // ============================================================================
        // CRITICAL FIX 3: Add unique constraints for data integrity
        // ============================================================================

        // Ensure inventory is unique per product-warehouse-company combination
        await queryRunner.query(`
            CREATE UNIQUE INDEX "UQ_inventory_product_warehouse_company" 
            ON "inventory" ("product_id", "warehouse_id", "company_id");
        `);

        // Ensure invoice numbers are unique per company
        await queryRunner.query(`
            ALTER TABLE "invoices" 
            ADD CONSTRAINT "UQ_invoices_number_company" 
            UNIQUE ("invoiceNumber", "customerId");
        `);

        // ============================================================================
        // CRITICAL FIX 4: Add missing indexes for performance
        // ============================================================================
